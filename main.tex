...
\section{Context}
\begin{frame}{Context}
  Study of wheat grain development.
  
  Molecules involved in water transfers \cite{Fanuel18} :
...
\begin{frame}{New data}
  New data : \textbf{Bobwhite}, 3 stages of development.
...
\begin{frame}{3D workflow}
  ...
  \begin{enumerate}
  \item<2-> MALDI: spectra \textbf{normalization}
  \item<3-> \textbf{3D registration}
  \item<4-> Spatial correlations with \textbf{uncertainties}
  \item<5-> \textbf{Visualization} of 3D MS images
...
\begin{frame}{Deformable registration}
  ...
  \textbf{New metric}: sum of squared differences with an update on DT values.
...
\begin{frame}{Correlative analysis}
  ...
  Results on the \textbf{Bobwhite} wheat grain family (250 DAA stage):
  ...
  \end{figure}
   ...
   $\Rightarrow$ $\dfrac{\text{AX6}}{\text{AX5}}$ ratio: link between AX \textbf{substitution} and \textbf{water mobility} \cite{Fanuel18}.
...
\begin{frame}{Conclusion}
  ...
  Adjusted workflow for the \textbf{automatic} processing and analysis of 3D MS images.

  Suited for 3D MS image fusion with homogeneous or inhomogeneous intensities.
